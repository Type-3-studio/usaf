...
class TerminalReporter(BaseReporter):
    ...
    def print_to_console(self, result: ScanResult, score: ScanScore | None = None, **kwargs: Any) -> None:
    ...
    def _print_score(self, console: Console, score: ScanScore | None) -> None:
        ...
        grade_color = "green" if score.overall_score < 2 else (
            "yellow" if score.overall_score < 5 else "red"
        )
    ...
    def _print_finding(self, console: Console, finding: Finding) -> None:
        ...
        console.print(f"    [dim]Score:[/] {finding.risk_score}  [dim]Confidence:[/] {finding.confidence.value}  "
        ...
        if finding.evidence:
            ...
            if isinstance(ev, FileEvidence):
                ...
                console.print(f"      {ev.protocol} {ev.local_address}:{ev.local_port} → "
    ...
    def _print_checks_summary(self, console: Console, result: ScanResult, show_passed: bool) -> None:
        if not show_passed:
            return
    ...
    def _print_footer(self, console: Console, result: ScanResult) -> None:
        console.print(f"[dim]Report generated by USAF v{result.metadata.usaf_version or '0.1.0'} "
